...
from utils.helpers import ensure_dir, softmax_sample, allocate_seats_largest_remainder
...
class PelotasElectionABM:
    ...
    def _precompute_candidate_arrays(self) -> None:
        ...
        # Build territorial strength matrix
        ...
        for t in range(args.n_steps):
            ...
            for i in range(self.n_voters):
                ...
                viability_term = args.kappa_viability * args.f_strategic * (
                    args.rho1 * candidate_share_prev +
                    args.rho2 * social_by_candidate +
                    args.rho3 * viability_party
                )

                noise = rng.normal(0.0, args.noise_sigma, self.n_candidates)
                
                utility = (ideology_term + visibility_term + territorial_term + 
                          social_term + party_term + incumbency_term + viability_term + noise)
